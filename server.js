...
const handlers = {
 ...
 log(`request made on /login from ${req.socket.localAddress} ` + Date(Date.now()).toString());
  try {
   file = fs.readFileSync(__dirname + "/public/pages/index.html", "utf-8").replace("{title}", "Centro: Login").replace("{bundle}", "/dist/login.bundle.js");
  } catch (err) {
   handlers["404"](req, res, "file not found!");
  }
  res.setHeader("Content-Type", "text/html");
 ...
 log(`request made on /checkout from ${req.socket.localAddress} ` + Date(Date.now()).toString());
  try {
   file = fs.readFileSync(__dirname + "/public/pages/index.html", "utf-8").replace("{title}", "Centro: Checkout").replace("{bundle}", "/dist/checkout.bundle.js");
  } catch (err) {
   handlers["404"](req, res, "file not found!");
  }
  res.setHeader("Content-Type", "text/html");
